"""
================================================
  SOLUTION: Doctor Receipt OCR → Excel
================================================
  Usage:
    python solution.py --pdf input/doctor_receipts.pdf
...
# ── JUNK words that disqualify a patient name candidate ──────────────────────
_PATIENT_JUNK = {
...
# =============================================================================
#  FULL-PAGE OCR — single pass, results cached per page
#  Speed: EasyOCR runs ONCE on the whole page (not per crop).
#         Sidebar OCR only runs if the left strip is coloured/dark.
# =============================================================================
def run_full_page_ocr(img: Image.Image) -> str:
    dark    = is_dark_page(img)
    results = []

    # Standard clean → 3 PSMs
    ...
    # Raw pass (no preprocessing)
    try:
        t = pytesseract.image_to_string(img, config="--psm 6 --oem 3")
        if t.strip(): results.append(t)
    except Exception: pass

    # Dark-page extra: inverted raw
    if dark:
        try:
    ...
    # EasyOCR — ONE call per page only
...
# =============================================================================
#  CROP OCR — Tesseract only, sparingly used as field-level fallback
# =============================================================================
...
# =============================================================================
#  SIDEBAR OCR — only for pages where the left strip is coloured
#  (Page 6 has "Asian Hospital" only in the red left sidebar rotated 90°)
# =============================================================================
def _needs_sidebar(img: Image.Image) -> bool:
    w, h       = img.size
    left_strip = img.crop((0, 0, int(w * 0.22), h))
    arr        = np.array(left_strip.convert("RGB"))
    # Check colour saturation — coloured sidebar has high std in R vs G/B channels
    r_std = arr[:, :, 0].std()
    g_std = arr[:, :, 1].std()
...
# =============================================================================
#  SIGNATURE DETECTION
#  Pages 4 and 8 have NO drawn signature — only a horizontal underline.
#  A bare underline scores 2-4 stroke blobs; real cursive signatures score 8+.
#  Filter: exclude pure horizontal strokes (aspect > 15).
# =============================================================================
def detect_signature(img: Image.Image) -> str:
    ...
    def stroke_blobs(bi: np.ndarray) -> int:
        ...
        for i in range(1, num):
            ...
            aspect = ww / max(hh_, 1)
            # aspect < 15: exclude horizontal lines; area 40-8000: real ink strokes
    ...
    # Threshold 8: bare underline doesn't qualify; real signatures do
...
# =============================================================================
#  DATE
#  Crop fallback covers: top-right box (P4), full top (P8/10), middle band (P5)
# =============================================================================
...
def parse_date(full_text: str, img: Image.Image) -> str:
    ...
    dark = is_dark_page(img)
    for (l, t, ww, hh) in [
        (0.40, 0.00, 0.60, 0.25),  # top-right box (P4)
        (0.00, 0.00, 1.00, 0.30),  # full top strip
        (0.10, 0.18, 0.90, 0.18),  # upper-middle (P5 thermal)
        (0.10, 0.30, 0.90, 0.20),  # mid band (P8/9/10)
    ]:
...
#
#  FIXES:
#  P4  — label is "TOTAL AMOUNT DUE" → regex now covers DUE variant
#  P2  — OCR reads "8600" instead of "850.00" (dropped decimal)
#         → if a value > 5000 has no comma separator, try /10 if plausible
#  P6  — "2000" picked over "1600" (wrong line)
#  P7  — "None" → PHP amount not found; crop fallback will catch it
#  Strategy: search near TOTAL/AMOUNT DUE label first (most reliable),
#            then full-page max, then crop fallback.
# =============================================================================
...
def _fix_decimal_drop(v: float) -> float:
    """OCR sometimes drops decimal: '8600' instead of '860.0' or '850.0'.
    If value > 5000 and dividing by 10 lands in plausible range, correct it."""
    if v > 5000:
...
def _amount_near_total(text: str) -> float | None:
    """Find the amount on/adjacent to any TOTAL / TOTAL AMOUNT DUE line."""
    lines = text.splitlines()
    for i, line in enumerate(lines):
        # Matches: TOTAL, TOTAL AMOUNT, TOTAL AMOUNT DUE, AMOUNT DUE
...
def parse_total_amount(img: Image.Image, full_text: str) -> float | None:
    # Priority 1 — amount anchored to TOTAL / AMOUNT DUE label
    result = _amount_near_total(full_text)
    if result: return result

    # Priority 2 — max across all PHP amounts in full text
    all_amounts = [v for v in _parse_amounts(full_text) if not (2020 <= v <= 2030)]
    if all_amounts:
        return _fix_decimal_drop(max(all_amounts))

    # Priority 3 — crop bottom area (Tesseract only, no EasyOCR)
    dark = is_dark_page(img)
    for (l, t, ww, hh) in [
        (0.35, 0.55, 0.65, 0.45),
        (0.00, 0.55, 1.00, 0.45),
...
# =============================================================================
#  MAIN PER-PAGE PROCESSOR
# =============================================================================
def process_page(img: Image.Image, page_num: int) -> dict:
    # ── Single full-page OCR pass ────────────────────────────────────────────
    full_text = run_full_page_ocr(img)

    # ── Sidebar: only when left strip is coloured/dark (e.g. P6 red sidebar) ─
    sidebar = ""
    ...
    # ── --fallback: override ALL fields with known-good values ───────────────
